...
for (const target of targetPages) {
  ...
  for (const mapPath of maps) {
    ...
    if (code) {
      const destPath = `src/app/${target}/page.tsx`;
      fs.writeFileSync(destPath, code, 'utf-8');
      console.log(`[SUCCESS] Recovered ${destPath} from ${mapPath} (${code.split('\n').length} lines)`);
      recovered = true;
      break;
    }
  }
  if (!recovered) {
    console.log(`[FAILED] Could not extract clean source for ${target}`);
  }
}
