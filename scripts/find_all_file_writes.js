    ...
    try {
      ...
      if (obj.tool_calls) {
        for (const tc of obj.tool_calls) {
          const argStr = tc.arguments || JSON.stringify(tc.args || {});
          if (argStr.includes('PlatformFeed.tsx')) {
            console.log(`Line ${count}: Step ${obj.step_index}, Tool ${tc.name}`);
            console.log(`  Args: ${argStr.slice(0, 300)}`);
          }
        }
      }
    } catch (e) {}
