                    ...
                    <div className="flex flex-col gap-1 max-w-[75%]">
                      ...
                      {msg.is_ai_generated && (
                        <span className={`text-[8px] font-black uppercase tracking-wider flex items-center gap-1 ${
                          isMe ? 'text-primary/70 justify-end mr-1' : 'text-violet-400/70 ml-1'
                        }`}>
                          <span className="inline-block w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse shrink-0" />
                          {isMe
                            ? '✦ Sent via your Replicant'
                            : `✦ Generated by ${selectedMatch?.target_profile?.display_name || selectedMatch?.target_profile?.username || 'Creator'}'s Replicant`
                          }
                        </span>
                      )}
                    </div>
