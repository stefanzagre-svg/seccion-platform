...
export async function POST(req: NextRequest) {
  try {
    ...
    let reply = "¡Hola! Estoy en modo demostración local, pero puedo decirte que SECCIØN se trata de encontrar conexiones auténticas sin algoritmos corporativos. ¿Listo para empezar tu quest?";
    ...
    if (geminiKey) {
      try {
        ...
        const systemPrompt = `
You are the Onboarding Specialist and Match Coach (main-character energy) for the SECCIØN platform.
...
5. Regulatory Delegation:
   - If the user asks deep regulatory compliance questions (e.g., CNMC ultimate beneficial ownership details, 18 U.S.C. 2257 records, CNMC, DAC7 tax structures, or GDPR details), explain that SECCIØN maintains absolute regulatory compliance and state-of-the-art verification (2257/DAC7) for creators to ensure 100% safety and legitimacy. Keep it clean and supportive.
`;
        ...
        reply = "Hey! My AI databases are undergoing a quick synchronization. SECCIØN is built on absolute safety, face blur encryption, and zero corporate algorithms. You can choose to start either the Member or Creator onboarding quest right here on the home page!";
      }
    }
